...
We then use the `xtabs` function to generate the contingency table:
```{r}
...
The two most common tests for determining whether different groups are subject to different distributions are the chi-squared test ($\chi^2$ test) and Fisher's exact test.
