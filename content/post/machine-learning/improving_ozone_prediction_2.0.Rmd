...
To understand the autocorrelation, we need to introduce the backshift operator.

Given the time series $X = \{X_1, X_2, \ldots \}$, the lag operator is defined as

\[L X_t = X_{t-1}\,, \forall t > 1\]

Instead of $L$, $B$ is sometimes used equivalently to indicate the backshift operator. 

Raising the lag operator to a power $k > 0$ performs multiple shifts at once:
