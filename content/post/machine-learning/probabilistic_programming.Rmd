...
Since we are not only interested in learning about the data but about making predictions for new data points, we need to sample from the posterior predictive distribution. 

Fit and predict scheme:
...
param.sample <- as.data.frame(fit1)  
# generate some artificial new data for prediction:
# assume we have four more samples for the first school!
newdata <- data.frame(y = c(50, 30, 10, 80),
                    sigma = c(20, 10, 3, 25)) 
# posterior predicted values for coaching effect on newdata
predict.custom <- apply(param.sample, 1, function(x) x["mu"] + x["tau"] * x["eta[1]"])
rnorm(1, param.sample[1, "theta[1]"], newdata$sigma[1])
result.sample <- t(apply(param.sample, 1, function(x) 
