...
The reason why PCA is so popular is that it is very interpretable.
...
So, using $k$-nearest neighbors, PCA does not seem to boost the prediction accuracy for this data set. This is because there are few features to begin with, so the dimensionality reduction does not help much. Moreover, there are few low-variance features in the data set (e.g. ```Tobacco``` or ```Malty```):
