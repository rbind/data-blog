...
Before we begin reducing the dimensionality of the data, we should think about what kind of results we would like to obtain. For the whiskey data, we would expect that whiskeys with similar taste profiles are close to each other in the reduced space. Since whiskeys from distilleries that are in proximity to another use similar distilling techniques and resources, their whiskeys are also often similar. Thus, a reasonable dimensionality reduction should cluster whiskeys from similar regions. To validate this assumption, we are going to test whether the mean expression of whiskey characteristics differ between distilleries from different regions. For this, we will run a MANOVA test:
...
So, using $k$-nearest neighbors, PCA indeed boosts the prediction accuracy from `r paste0(round(accuracy.knn, 3)*100, "%")` to `r paste0(round(accuracy.pca.knn, 3) * 100, "%")`. Why does PCA help for learning from this data set? This is because there are some features that are not very helpful for differentiating the whiskeys. Let us look of the feature variances:
...
For example, ```Tobacco``` has a very low variance because most whiskeys do not exhibit this trait (mean value of `r round(mean(data$Tobacco), 2)`.

Now that we are able to identify the six regions of Scottish whiskey with an accuracy close to 60% by considering 12 whiskey taste characteristics, the question is if we can still do better. We know that it is hard to predict the underrepresented regions correctly. So, can we do better if we limit ourselves to fewer regions? From the PCA analysis, we could regroup the labels in the following way:
...
With an accuracy of `r paste0(round(accuracy.pca.knn, 3)*100, "%")`, we can conclude that this approach did not work too well. That is probably because the whiskeys from different groups are too dissimilar to be merged.
